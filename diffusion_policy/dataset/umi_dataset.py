...
class UmiDataset(BaseDataset):
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for key in self.lowdim_keys:
            ...
            if key.endswith('pos') or 'pos_wrt' in key:
                ...
                this_normalizer = get_range_normalizer_from_stat(stat)
            elif key.endswith('patch_xyz'):
                this_normalizer = get_range_normalizer_from_stat(stat)
            elif key.endswith('patch_valid'):
                this_normalizer = get_identity_normalizer_from_stat(stat)
            else:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.sampler_lowdim_keys:
            ...
            # flatten patch observations for shape_meta compatibility
            if key.endswith('patch_xyz'):
                # (T,16,16,3) -> (T,768)
        ...
        # remove demo start/end pose from obs
        del_keys = list()
        for key in obs_dict:
            if key.endswith('_demo_start_pose') or key.endswith('_demo_end_pose'):
                del_keys.append(key)
        ...
        # build action and relative obs/action pose
        actions = list()
        for robot_id in range(self.num_robot):
            pose_mat = pose_to_mat(np.concatenate([
                obs_dict[f'robot{robot_id}_eef_pos'],
