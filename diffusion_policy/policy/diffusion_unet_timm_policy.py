...
class DiffusionUnetTimmPolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        self.kwargs = kwargs
        self.loss_weighting = loss_weighting or {} #WZM: new
    ...
    def compute_loss(self, batch):
        ...
        # ---- reduce to per-step loss: (B,T)
        loss_bt = loss.mean(dim=-1)

        # ---- optional weighting by ||Δpos||
        lw = self.loss_weighting
        if lw.get("enable", False):
            # batch['action'] is UN-normalized action in physical units (relative pose in meters)
            # shape: (B,T,10) => first 3 dims are Δpos
            dpos = torch.linalg.norm(batch['action'][..., 0:3], dim=-1)  # (B,T)

            thr = float(lw.get("thr_dpos", 0.162))     # use your P95 by default
            alpha = float(lw.get("alpha", 4.0))        # strength
            w_max = float(lw.get("w_max", 6.0))        # cap
            eps = 1e-8

            #w = 1.0 + alpha * torch.clamp(dpos / (thr + eps), 0.0, max(0.0, w_max - 1.0))
            ratio = dpos / (thr + eps)
            ratio = torch.clamp(ratio, min=0.0, max=max(0.0, w_max - 1.0))
            w = 1.0 + alpha * ratio
            # (B,T) weight

            # If train_diffusion_n_samples repeats batch dim, repeat weights as well
            if self.train_diffusion_n_samples != 1:
                w = torch.repeat_interleave(w, repeats=self.train_diffusion_n_samples, dim=0)
