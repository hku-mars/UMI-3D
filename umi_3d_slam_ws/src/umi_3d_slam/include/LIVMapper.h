...
public:
  ...
  void publish_camera_odometry(const ros::Publisher &pubOdomCamToInit);

  // NEW: publish camera path (anchored to camera pose)
  void publish_camera_path(const ros::Publisher pubPathCam);
  ...
  // FOV / angle filter
  bool fov_filter_en_ = false;
  double yaw_min_deg_ = -180.0;
  double yaw_max_deg_ =  180.0;
  bool pitch_filter_en_ = false;
  double pitch_min_deg_ = -90.0;
  double pitch_max_deg_ =  90.0;

  inline static double deg2rad(double d) { return d * M_PI / 180.0; }
  bool angleInRange(double ang, double a_min, double a_max) const;
  bool keepPointByAngle(float x, float y, float z) const;
  void applyAngleFilter(PointCloudXYZI::Ptr& cloud) const;
  ...
  // NEW: camera path (anchored to camera pose from publish_camera_odometry)
  ...
  ros::Publisher pubOdomCamToInit;//WZM
  ros::Publisher pubPath;

  // NEW: camera path publisher
  ros::Publisher pubPathCam;
  ...
  //wzm: cut time pub
  ros::Publisher pub_cut_stamp_;
  std::string cut_stamp_topic_;
  bool pub_cut_stamp_en_{true};
