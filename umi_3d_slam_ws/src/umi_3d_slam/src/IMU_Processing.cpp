                           ...
                           Zero3d(0, 0, 0), b_first_frame(true), imu_need_init(true)
{
  ...
  cur_pcl_un_.reset(new PointCloudXYZI());
    // --- publish IMU init status (global, latched) ---
  ros::NodeHandle nh; // global namespace
  imu_init_pub_ = nh.advertise<std_msgs::Bool>("/imu_initialized", 1, true); // latched
  imu_init_pub_inited_ = true;

  std_msgs::Bool m;
  m.data = false;
  imu_init_pub_.publish(m);
  imu_initialized_ = false;

  livo_lost_pub_ = nh.advertise<std_msgs::Bool>("/livo_frame_lost", 1000, false);
  livo_lost_pub_inited_ = true;

  // 启动时先发一个 true（更安全：还没开始就认为 lost）
  std_msgs::Bool ml;
  ml.data = true;
  livo_lost_pub_.publish(ml);
  last_lost_ = true;
}

void ImuProcess::publish_lost(bool lost)
{
  last_lost_ = lost;
  if (!livo_lost_pub_inited_) return;
  std_msgs::Bool m;
  m.data = lost;
  livo_lost_pub_.publish(m);
}
...
{
  std::cout << "[ DEBUG ] lidar_meas.pcl_proc_cur size: " << lidar_meas.pcl_proc_cur->size() << std::endl;
  std::cout << "[ DEBUG ] lidar_meas.lidar size: " << lidar_meas.lidar->size() << std::endl;
  ...
  if (lidar_meas.lio_vio_flg == LIO)
  {
    ...
    std::cout << "[ DEBUG ] pcl_wait_proc 点数: " << pcl_wait_proc.size() << std::endl;
  ...
  std::cout << "[ DEBUG ] 最终输出点数 pcl_out: " << pcl_out.size() << std::endl;
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (!imu_en)
  {
    ...
    // 没 IMU 模式：只要点云非空就不算 lost（你也可以永远 true）
    publish_lost(cur_pcl_un_->empty());
    return;
  }

  MeasureGroup meas = lidar_meas.measures.back();

  // ---- Case 0: 本帧 IMU 为空（你日志第一帧就是这样）----
  if (meas.imu.empty())
  {
    publish_lost(true);
    return;
  ...
  // ---- Case 1: 初始化阶段（你希望也要记录，并标 lost）----
  if (imu_need_init)
  {
    std::cout << "[ DEBUG ] 当前帧IMU数量 meas.imu.size(): " << meas.imu.size() << std::endl;
    ...
    // 初始化阶段这一帧仍然不做 Undistort，所以按你的需求：必 lost
    publish_lost(true);
    return;
  }
  ...
  {
    std::cout << "[ DEBUG ] UndistortPcl 输出为空！feats_undistort size = 0" << std::endl;
    publish_lost(true);
  }
  else
  {
    std::cout << "[ DEBUG ] UndistortPcl 成功，输出点数: " << cur_pcl_un_->size() << std::endl;
    publish_lost(false);
  }
}
