      ...
      extR(M3D::Identity())
{
  ...
  // NEW: camera path
  path_cam.header.stamp = ros::Time::now();
  path_cam.header.frame_id = "camera_init";
}
...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<int>("preprocess/cut_frame_num", cut_frame_num_, 1);
  if (cut_frame_num_ < 1) cut_frame_num_ = 1;
  ...
  //WZM: FOV
  nh.param<bool>("fov_filter/enable", fov_filter_en_, false);
  nh.param<double>("fov_filter/yaw_min_deg", yaw_min_deg_, -180.0);
  nh.param<double>("fov_filter/yaw_max_deg", yaw_max_deg_,  180.0);

  nh.param<bool>("fov_filter/pitch_enable", pitch_filter_en_, false);
  nh.param<double>("fov_filter/pitch_min_deg", pitch_min_deg_, -90.0);
  nh.param<double>("fov_filter/pitch_max_deg", pitch_max_deg_,  90.0);

  p_pre->blind_sqr = p_pre->blind * p_pre->blind;

  //WZM: cut time
  nh.param<bool>("publish/pub_cut_stamp_en", pub_cut_stamp_en_, true);
  nh.param<string>("publish/cut_stamp_topic", cut_stamp_topic_, string("/livo/cut_stamp"));
}
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  pubOdomCamToInit = nh.advertise<nav_msgs::Odometry>("/camera_to_init", 10);  //WZM:添加相机到camera_init的odom
  pubPath = nh.advertise<nav_msgs::Path>("/path", 10);
  pubPathCam = nh.advertise<nav_msgs::Path>("/path_camera", 10); //WZM:publish camera-anchored path
  ...
  if (pub_cut_stamp_en_) {
    pub_cut_stamp_ = nh.advertise<std_msgs::Time>(cut_stamp_topic_, 1000);
  }
}
...
{    
  ...
  publish_camera_odometry(pubOdomCamToInit); //WZM
  publish_camera_path(pubPathCam);
...
//WZM: FOV
bool LIVMapper::angleInRange(double ang, double a_min, double a_max) const
{
  // ang, a_min, a_max are all in radians, normalized to (-pi, pi]
  // support wrap-around, e.g. [170deg, -170deg] means "near 180deg" sector
  if (a_min <= a_max) return (ang >= a_min && ang <= a_max);
  // wrap case
  return (ang >= a_min || ang <= a_max);
}

bool LIVMapper::keepPointByAngle(float x, float y, float z) const
{
  // yaw
  double yaw = std::atan2((double)y, (double)x); // (-pi, pi]
  double yaw_min = deg2rad(yaw_min_deg_);
  double yaw_max = deg2rad(yaw_max_deg_);
  if (!angleInRange(yaw, yaw_min, yaw_max)) return false;

  if (pitch_filter_en_)
  {
    double xy = std::sqrt((double)x * x + (double)y * y);
    double pitch = std::atan2((double)z, xy); // (-pi/2, pi/2)
    double pmin = deg2rad(pitch_min_deg_);
    double pmax = deg2rad(pitch_max_deg_);
    if (pitch < pmin || pitch > pmax) return false;
  }
  return true;
}

void LIVMapper::applyAngleFilter(PointCloudXYZI::Ptr& cloud) const
{
  if (!fov_filter_en_ || !cloud || cloud->empty()) return;
  ...
  {
    // pt is in LiDAR frame here (after preprocess)
    if (keepPointByAngle(pt.x, pt.y, pt.z))
...
//**** WZM分帧
static void splitCloudByTime(
    const PointCloudXYZI::Ptr& in,
    int cut_num,
    double header_time_sec,
    std::vector<PointCloudXYZI::Ptr>& out_clouds,
    std::vector<double>& out_header_times_sec)
{
  out_clouds.clear();
  out_header_times_sec.clear();
  if (!in || in->empty() || cut_num <= 1) {
    out_clouds.push_back(in);
    out_header_times_sec.push_back(header_time_sec);
    return;
  }

  // 要求：in 已经按 curvature(=offset ms) 递增（一般 preprocess 已经是时间序）
  // 如果不确定，也可以复制后 sort，但会增加开销；建议先假设已排序。

  const double t0_ms = in->points.front().curvature;
  const double t1_ms = in->points.back().curvature;
  const double T_ms  = t1_ms - t0_ms;
  ...
  const double dt_ms = T_ms / double(cut_num);

  int seg_id = 0;
  double seg_start_ms = t0_ms;

  PointCloudXYZI::Ptr cur(new PointCloudXYZI());
  cur->reserve(in->size() / cut_num + 64);

  auto flush_seg = [&](int sid, double seg_start_ms_local){
    if (!cur->empty()) {
      // 子帧的 header 时间（绝对秒）
      double sub_header_sec = header_time_sec + seg_start_ms_local / 1000.0;
      out_header_times_sec.push_back(sub_header_sec);

      // 关键：把点的 curvature 改成“相对子帧帧头”的 offset ms（从0开始）
      for (auto &pt : cur->points) {
        pt.curvature = pt.curvature - seg_start_ms_local;
      }
      out_clouds.push_back(cur);
    }
    cur.reset(new PointCloudXYZI());
    cur->reserve(in->size() / cut_num + 64);
  };

  for (size_t i = 0; i < in->points.size(); ++i) {
    const double pt_ms = in->points[i].curvature;

    // 推进到所属段（避免跨越导致的空段）
    while (seg_id < cut_num - 1 && pt_ms >= (t0_ms + (seg_id + 1) * dt_ms)) {
      flush_seg(seg_id, seg_start_ms);
      seg_id++;
      seg_start_ms = t0_ms + seg_id * dt_ms;
    }
    cur->points.push_back(in->points[i]);
  }
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  applyAngleFilter(ptr);   // <<< WZM新增：角度过滤
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver::CustomMsg::ConstPtr &msg_in)
{
  ...
  //lid_raw_data_buffer.push_back(ptr);
  //lid_header_time_buffer.push_back(cur_head_time); //原来的
  if (!ptr || ptr->empty()) {
    ROS_ERROR("Received an empty point cloud");
    mtx_buffer.unlock();
    return;
  ...
  // 切帧：10Hz -> cut_frame_num_ * 10Hz
  std::vector<PointCloudXYZI::Ptr> sub_clouds;
  std::vector<double> sub_headers;
  splitCloudByTime(ptr, cut_frame_num_, cur_head_time, sub_clouds, sub_headers);

  // 入队：每个子帧都像“独立一帧 lidar”一样进入 buffer
  for (size_t k = 0; k < sub_clouds.size(); ++k) {
    if (!sub_clouds[k] || sub_clouds[k]->empty()) continue;

    lid_raw_data_buffer.push_back(sub_clouds[k]);
    lid_header_time_buffer.push_back(sub_headers[k]);

    if (pub_cut_stamp_en_ && pub_cut_stamp_) {
      std_msgs::Time tmsg;
      tmsg.data = ros::Time().fromSec(sub_headers[k]);
      pub_cut_stamp_.publish(tmsg);
    }
  }
  //last_timestamp_lidar = cur_head_time; // <-- 这一句会把它又改回 10Hz
  if (!sub_headers.empty()) {
    last_timestamp_lidar = sub_headers.back();
  } else {
    last_timestamp_lidar = cur_head_time;
  }
...
void LIVMapper::publish_camera_odometry(const ros::Publisher &pubOdomCamToInit)
{
  nav_msgs::Odometry odomCam;
  odomCam.header.frame_id = "camera_init";
  odomCam.child_frame_id = "camera";
  odomCam.header.stamp = odomAftMapped.header.stamp;

  // 1) ^w T_i (IMU pose in camera_init)
  Eigen::Matrix3d R_w_i = _state.rot_end;
  Eigen::Vector3d t_w_i(_state.pos_end(0), _state.pos_end(1), _state.pos_end(2));

  // 2) ^i T_l (LiDAR -> IMU)
  Eigen::Matrix3d R_i_l = extR;
  Eigen::Vector3d t_i_l(extT(0), extT(1), extT(2));

  // 3) ^c T_l (LiDAR -> Camera) from Rcl/Pcl
  Eigen::Matrix3d R_c_l;
  Eigen::Vector3d t_c_l;
  R_c_l << MAT_FROM_ARRAY(cameraextrinR);
  t_c_l << VEC_FROM_ARRAY(cameraextrinT);

  // invert: ^l T_c
  // invert: ^l T_c
  Eigen::Matrix3d R_l_c = R_c_l.transpose();
  Eigen::Vector3d t_l_c = -R_l_c * t_c_l;


  // 4) compose: ^w T_c = ^w T_i * ^i T_l * ^l T_c
  Eigen::Matrix3d R_w_l = R_w_i * R_i_l;
  Eigen::Vector3d t_w_l = R_w_i * t_i_l + t_w_i;

  Eigen::Matrix3d R_w_c = R_w_l * R_l_c;
  Eigen::Vector3d t_w_c = R_w_l * t_l_c + t_w_l;

  // 5) fill odom
  Eigen::Quaterniond q_w_c(R_w_c);
  odomCam.pose.pose.position.x = t_w_c.x();
  odomCam.pose.pose.position.y = t_w_c.y();
  odomCam.pose.pose.position.z = t_w_c.z();
  odomCam.pose.pose.orientation.x = q_w_c.x();
  odomCam.pose.pose.orientation.y = q_w_c.y();
  odomCam.pose.pose.orientation.z = q_w_c.z();
  odomCam.pose.pose.orientation.w = q_w_c.w();

  pubOdomCamToInit.publish(odomCam);

  // WZM: fill camera pose stamped for /path_camera
  cam_pose_stamped.header.stamp = odomCam.header.stamp;
  cam_pose_stamped.header.frame_id = "camera_init";
  cam_pose_stamped.pose.position.x = t_w_c.x();
  cam_pose_stamped.pose.position.y = t_w_c.y();
  cam_pose_stamped.pose.position.z = t_w_c.z();
  cam_pose_stamped.pose.orientation.x = q_w_c.x();
  cam_pose_stamped.pose.orientation.y = q_w_c.y();
  cam_pose_stamped.pose.orientation.z = q_w_c.z();
  cam_pose_stamped.pose.orientation.w = q_w_c.w();

  // optional TF
  static tf::TransformBroadcaster br;
  tf::Transform tf_cam;
  tf_cam.setOrigin(tf::Vector3(t_w_c.x(), t_w_c.y(), t_w_c.z()));
  tf_cam.setRotation(tf::Quaternion(q_w_c.x(), q_w_c.y(), q_w_c.z(), q_w_c.w()));
  br.sendTransform(tf::StampedTransform(tf_cam, odomCam.header.stamp, "camera_init", "camera"));
}
...
void LIVMapper::publish_camera_path(const ros::Publisher pubPathCam)
{
  path_cam.header.stamp = cam_pose_stamped.header.stamp;
  path_cam.header.frame_id = "camera_init";
  path_cam.poses.push_back(cam_pose_stamped);
  pubPathCam.publish(path_cam);
}
