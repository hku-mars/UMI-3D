...
def main(input, output, out_res, out_fov, compression_level,
    ...
    # -------------------------------
    # NEW (minimal): load principal point (cx, cy) from fisheye.json
    # -------------------------------
    cxcy = None
    fisheye_json_path = os.path.join(ROOT_DIR, 'example', 'calibration', 'fisheye.json')
    if os.path.isfile(fisheye_json_path):
        try:
            with open(fisheye_json_path, 'r') as f:
                fisheye_intr = json.load(f)
            cx = float(fisheye_intr['intrinsics']['principal_pt_x'])
            cy = float(fisheye_intr['intrinsics']['principal_pt_y'])
            cxcy = (cx, cy)
            print(f"[INFO] Using principal point from fisheye.json: cx={cx:.3f}, cy={cy:.3f}")
        except Exception as e:
            print(f"[WARN] Failed to load principal point from {fisheye_json_path}: {e}")
            cxcy = None
    else:
        print(f"[WARN] fisheye.json not found at {fisheye_json_path}. Fallback to image-center crop.")
        cxcy = None

    # -------------------------------
    # Optional fisheye rectify/reproject (unchanged behavior if you don't pass --out_fov)
    # NOTE: fixed a small bug: original code referenced undefined ipath here.
    # -------------------------------
    fisheye_converter = None
    if out_fov is not None:
        intr_path = pathlib.Path(ROOT_DIR).joinpath(
    ...
    if len(vid_args) == 0:
        raise RuntimeError("No valid videos found. Check your input paths and dataset_plan.pkl.")
    ...
    def video_to_zarr(replay_buffer, mp4_path, tasks):
        ...
        # -------------------------------
        # NEW (minimal): pass crop_center_xy=(cx, cy) into get_image_transform
        # -------------------------------
        ...
        with av.open(mp4_path) as container:
            ...
            for frame_idx, frame in tqdm(enumerate(container.decode(in_stream)), total=in_stream.frames, leave=False):
                ...
                if frame_idx < tasks[curr_task_idx]['frame_start']:
                    ...
                    raise AssertionError("Frame index logic error.")
