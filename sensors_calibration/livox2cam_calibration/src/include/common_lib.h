...
Params loadParameters(ros::NodeHandle &nh) {
  Params params;
  nh.param("fx", params.fx, 396.11028613);
  nh.param("fy", params.fy, 395.17819253);
  nh.param("cx", params.cx, 668.35571792);
  nh.param("cy", params.cy, 477.47812051);
  nh.param("k1", params.k1, -0.01743796);
  nh.param("k2", params.k2, 0.0082378);
  nh.param("p1", params.k3, -0.01465171);
  nh.param("p2", params.k4, 0.00525416);
...
//   }
// }
// Fisheye projection version (RAW fisheye image domain)
void projectPointCloudToImage_fisheye(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
    const Eigen::Matrix4f& T_cam_lidar,          // transformation: lidar -> camera
    const cv::Mat& K,                            // 3x3
    const cv::Mat& D,                            // 4x1 (k1,k2,k3,k4) for cv::fisheye
    const cv::Mat& image_bgr,                    // RAW fisheye image (BGR)
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr& colored_cloud,
    bool debug = false,
    int max_points = -1                          // optional: limit for speed (<=0 means no limit)
)
{
    colored_cloud->clear();
    colored_cloud->reserve(cloud->size());

    // Ensure type is correct for fisheye API
    cv::Mat Kd, Dd;
    K.convertTo(Kd, CV_64F);
    D.convertTo(Dd, CV_64F);

    // rvec/tvec are zero because points already in camera frame after applying T_cam_lidar
    ...
    // For speed, we can process in small batches (fisheye::projectPoints supports vector)
    std::vector<cv::Point3f> pts_cam;
    pts_cam.reserve(4096);

    std::vector<Eigen::Vector3f> pts_cam_eig;
    pts_cam_eig.reserve(4096);

    auto flush_batch = [&](){
        if (pts_cam.empty()) return;

        std::vector<cv::Point2f> uv;
        uv.resize(pts_cam.size());

        // cv::fisheye::projectPoints(objectPoints, imagePoints, rvec, tvec, K, D[, alpha[, jacobian]])
        cv::fisheye::projectPoints(pts_cam, uv, rvec, tvec, Kd, Dd);

        for (size_t i = 0; i < uv.size(); ++i) {
            int u = (int)std::lround(uv[i].x);
            int v = (int)std::lround(uv[i].y);
            n_project++;
            ...
            if ((unsigned)u < (unsigned)W && (unsigned)v < (unsigned)H) {
                ...
                pcl::PointXYZRGB cp;
                cp.x = pts_cam_eig[i].x();
                cp.y = pts_cam_eig[i].y();
                cp.z = pts_cam_eig[i].z();
                cp.r = color[2];
                cp.g = color[1];
                cp.b = color[0];
    ...
    for (const auto& p : *cloud) {
        if (max_points > 0 && n_total >= max_points) break;
        n_total++;

        Eigen::Vector4f ph(p.x, p.y, p.z, 1.0f);
        Eigen::Vector4f pc = T_cam_lidar * ph;

        // Keep only points in front of camera
        if (pc.z() <= 0.0f) continue;
        n_front++;

        pts_cam.emplace_back(pc.x(), pc.y(), pc.z());
        pts_cam_eig.emplace_back(pc.x(), pc.y(), pc.z());

        // flush in batches for speed
        if (pts_cam.size() >= 4096) flush_batch();
    }

    flush_batch();

    if (debug) {
        ROS_INFO("[FisheyeColor] total=%d front(z>0)=%d projected=%d in_img=%d colored=%zu",
                 n_total, n_front, n_project, n_in_img, colored_cloud->size());
        ROS_INFO("[FisheyeColor] image size: %dx%d  K=(fx=%.3f fy=%.3f cx=%.3f cy=%.3f)  D=[%.6f %.6f %.6f %.6f]",
                 W, H,
                 Kd.at<double>(0,0), Kd.at<double>(1,1),
                 Kd.at<double>(0,2), Kd.at<double>(1,2),
                 Dd.at<double>(0,0), Dd.at<double>(1,0),
                 Dd.at<double>(2,0), Dd.at<double>(3,0));
    }
}
