...
#     return img
# =========== image mask (ADAPTED FOR 1024x1280 CAMERA) ====================
MASK_TUNED_PARAMS = {
  "enable_mirror": True,
  "enable_gripper": True,
  "enable_finger": True,
  "mirror": {"dx": 0.017500, "dy": 0.500000, "s": 1.100000},
  "gripper": {"dx": 0.035500, "dy": -0.293000, "s": 1.710000},
  "finger": {"dx": 0.02, "dy": -0.008000, "s": 0.960000},
  "finger_height": 0.45,
  "finger_top_width": 0.250000,
  "finger_bottom_width": 1.400000,
}

def _apply_mask_affine(coords, group_name):
    """
    Apply tuned canonical affine transform to polygon coords.
    coords: (N,2) canonical
    """
    if group_name not in MASK_TUNED_PARAMS:
        return coords

    p = MASK_TUNED_PARAMS[group_name]
    dx, dy, s = p["dx"], p["dy"], p["s"]
...
# Your camera native resolution (H, W)
_DEFAULT_RESOLUTION = (1024, 1280)

def canonical_to_pixel_coords(coords, img_shape=_DEFAULT_RESOLUTION):
    H, W = img_shape
    pts = np.asarray(coords) * H + np.array([W * 0.5, H * 0.5])
    return pts

def pixel_coords_to_canonical(pts, img_shape=_DEFAULT_RESOLUTION):
    H, W = img_shape
    coords = (np.asarray(pts) - np.array([W * 0.5, H * 0.5])) / H
    return coords


def get_mirror_canonical_polygon(resolution=_DEFAULT_RESOLUTION):
    left_pts = np.array([
        [230, 858],
        [301, 732],
        [255, 540],
    ...
    left_coords = pixel_coords_to_canonical(left_pts, img_shape=resolution)
    right_coords = left_coords.copy()
    right_coords[:,0] *= -1
    return np.stack([left_coords, right_coords])


def get_mirror_crop_slices(img_shape=_DEFAULT_RESOLUTION, left=True):
    resolution = _DEFAULT_RESOLUTION
    left_rect_pts = np.array([
        [104, 566],
        [286, 747]
    ], dtype=np.float64)

    left_coords = pixel_coords_to_canonical(left_rect_pts, img_shape=resolution)
    if not left:
        left_coords[:,0] *= -1

    pts = canonical_to_pixel_coords(left_coords, img_shape=img_shape)
    pts = np.round(pts).astype(np.int32)

    ys = pts[:,1]
    xs = pts[:,0]
    return (
        slice(int(np.min(ys)), int(np.max(ys))),
        slice(int(np.min(xs)), int(np.max(xs)))
    )


def get_gripper_canonical_polygon(resolution=_DEFAULT_RESOLUTION):
    left_pts = np.array([
        [640, 874],
        [513, 858],
        [286, 757],
...
def get_finger_canonical_polygon(height=0.37, top_width=0.25, bottom_width=1.4,
    ...
    return pixel_coords_to_canonical(points, img_shape=resolution)


def draw_predefined_mask(img, color=(0,0,0), mirror=True, gripper=True, finger=True, use_aa=False):
    """
    Same call signature as before.
    Only difference: apply tuned canonical affine before rasterization.
    """

    flag = cv2.LINE_AA if use_aa else cv2.LINE_8

    # ---------- mirror ----------
    if mirror and MASK_TUNED_PARAMS["enable_mirror"]:
        for coords in get_mirror_canonical_polygon():
            coords2 = _apply_mask_affine(coords, "mirror")
            pts = canonical_to_pixel_coords(coords2, img.shape[:2])
            pts = np.round(pts).astype(np.int32)
            cv2.fillPoly(img, [pts], color=color, lineType=flag)

    # ---------- gripper ----------
    if gripper and MASK_TUNED_PARAMS["enable_gripper"]:
        for coords in get_gripper_canonical_polygon():
            coords2 = _apply_mask_affine(coords, "gripper")
            pts = canonical_to_pixel_coords(coords2, img.shape[:2])
            pts = np.round(pts).astype(np.int32)
            cv2.fillPoly(img, [pts], color=color, lineType=flag)

    # ---------- finger ----------
    if finger and MASK_TUNED_PARAMS["enable_finger"]:
        for coords in get_finger_canonical_polygon(
            height=MASK_TUNED_PARAMS["finger_height"],
            top_width=MASK_TUNED_PARAMS["finger_top_width"],
            bottom_width=MASK_TUNED_PARAMS["finger_bottom_width"]
        ):
            coords2 = _apply_mask_affine(coords, "finger")
            pts = canonical_to_pixel_coords(coords2, img.shape[:2])
            pts = np.round(pts).astype(np.int32)
            cv2.fillPoly(img, [pts], color=color, lineType=flag)
...
def get_image_transform(
    ...
    crop_center_xy=None,   # (cx, cy) in pixel coords, e.g. (668.36, 477.48)
):
    """
    Crop a region with the same aspect ratio as out_res, centered at crop_center_xy (if given),
    then resize to out_res.

    in_res: (iw, ih)
    out_res: (ow, oh)  # NOTE: cv2.resize expects (w, h)
    crop_center_xy: (cx, cy) in pixels on the original image
    """
    iw, ih = int(in_res[0]), int(in_res[1])
    ow, oh = int(out_res[0]), int(out_res[1])

    crop_ratio = float(crop_ratio)
    ch = int(round(ih * crop_ratio))
    cw = int(round(ih * crop_ratio / float(oh) * float(ow)))  # keep aspect ratio of output

    # safety clamp in case cw/ch exceed input
    ch = min(ch, ih)
    cw = min(cw, iw)
    ...
    if crop_center_xy is None:
        ...
        cx, cy = float(crop_center_xy[0]), float(crop_center_xy[1])
    ...
    w_slice = slice(x0, x0 + cw)
    h_slice = slice(y0, y0 + ch)
    ...
    def transform(img: np.ndarray):
        assert img.shape == (ih, iw, 3), f"Expected {(ih, iw, 3)} but got {img.shape}"
        img2 = img[h_slice, w_slice, c_slice]
        img2 = cv2.resize(img2, (ow, oh), interpolation=interp_method)
        return img2
